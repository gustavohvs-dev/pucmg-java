...
class Fila {
	...
	public void enfileirar(Musica novo) {
	...
	public double obterMediaDuration() {
		...
		for (int i = 0; i < tamanho; i++) {
			if (fila[i] != null) {
				media += fila[i % tamanho].getDuration_ms();
				count++;
			}
		}
		return media / count;
	...
	public void imprimir() {
		for (int i = frente; i < tras; i++) {
			System.out.println("[" + i + "] " + (i % tamanho));
			fila[i % tamanho].imprimir();
		}
	}
...
public class Exerc02 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < numberOfLinesRemaining; i++) {
			id = MyIO.readLine();
			if (id.charAt(0) == 'R') {
				System.out.println("(R) " + fila.desenfileirar().getName());
