...
//Essa classe é responsável por gerenciar a pilha de músicas implementada por meio de vetor
class Pilha {
	...
	/**
	 * Construtor sem params
	 */
	public Pilha() {
        this.posicaoPilha = -1;
        this.pilha = new Musica[9999];
    }

	/**
	 * Construtor com params
	 * @param numPosicoes tamanho do vetor
	 */
	public Pilha(int numPosicoes) {
        this.posicaoPilha = -1;
        this.pilha = new Musica[numPosicoes];
    }
	...
	public void empilhar(Musica musica) {
        if (this.posicaoPilha < this.pilha.length - 1) {
			this.posicaoPilha += 1;
            this.pilha[this.posicaoPilha] = musica;
        }
    }
	...
	public Musica desempilhar() {
        if (this.posicaoPilha == -1) {
			...
			Musica musicaRemovida = this.pilha[this.posicaoPilha];
...
public class Exerc01 {
	...
	public static void main(String[] args) {
		...
		Pilha pilha = new Pilha(numTotalMusicas); //Criando pilha
		...
		id = MyIO.readLine(); //Lê a primeira linha
		while (!id.equals("FIM")) {
			pesquisado = pesquisa.buscarId(listaMusicas, id); //Busca a música na lista de músicas pelo ID informado
			pilha.empilhar(pesquisado); //Empilha item pesquisado
			id = MyIO.readLine(); //Lê a próxima linha até encontrar a palavra FIM
		}
		id = MyIO.readLine(); //Lê a linha referente a quantidade de linhas a serem lidas posteriormente
