...
public class Exerc03 {
    ...
    public static void criarLog() {
        ...
        String nomeArquivo = String.format("%s_insercao.txt", matricula);
    ...
    public static void sort(Musica[] musica) {
        Insertion(musica, 100);
    }

    public static void Insertion(Musica[] musica, int n) {
        ...
        for (int i = 1; i < n; i++) {
            tmp = musica[i];
            int j = i - 1;
            while ((j >= 0) && (musica[j].getId().compareTo(tmp.getId()) > 0)) {
                musica[j + 1] = musica[j]; // Deslocamento
                j--;
            }
            musica[j + 1] = tmp;
        }
