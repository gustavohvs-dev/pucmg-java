...
public class Exerc02 {
    ...
    public static void selecao() {
        for (int i = 0; i < musicas.length; i++) {
            ...
            for (int j = i + 1; j < musicas.length; j++) {
                comparacoes++;
                if (musicas[j].getName().compareTo(musicas[menor].getName()) < 0
                        || (musicas[j].getName().equals(musicas[menor].getName())
                        && musicas[j].getId().compareTo(musicas[menor].getId()) < 0)) {
                    menor = j;
                }
            }
            swap(i, menor);
    ...
    public static void criarLog() {
        ...
        String nomeArquivo = String.format("%s_selecao.txt", matricula);
