...
class Musica {
	...
	private String[] getArtistas(String dados) {
		...
		for (int j = 0; j < artistas.length; j++) {
			...
			nomeArtista = "";
			inicio = 0;
			while ((artistas[j].charAt(inicio) == '\'') || (artistas[j].charAt(inicio) == ' ')) // percorrendo até
...
class TabelaHash {
	...
	// hashing linear
	// h_k(chave) = (chave mod M + k) mod M
	private int funcaoTransformacao(int chave, int k) {
		return (((chave % this.M) + k * (chave % 31)) % this.M);
	}
	...
	public void inserir(Musica novo) throws Exception {
		...
		while (!inseriu && (k < this.M)) {
			posicao = funcaoTransformacao(novo.getDuration_ms(), k);

			if (tabelaHash[posicao] == null) {
				tabelaHash[posicao] = novo;
				inseriu = true;
			} // else if (tabelaHash[posicao].getName() == novo.getValor())
				// throw new Exception("Erro ao tentar inserir o elemento de chave " +
				// novo.getValor() + " na tabela hash: o elemento já foi inserido
				// anteriormente!");
			else
				// colisão
				k++;
		...
		if (!inseriu)
			throw new Exception("Erro ao tentar inserir o elemento de chave " + novo.getDuration_ms()
					+ " na tabela hash: não há espaço na tabela hash!");
	}
	...
	public Musica pesquisar(int chave) {
		...
		Musica pesquisado = null;

		while (!encontrou && (k < this.M)) {
			posicao = funcaoTransformacao(chave, k);
			comparacao++;
			if ((tabelaHash[posicao] != null) && (tabelaHash[posicao].getDuration_ms() == chave)) {
				encontrou = true;
...
public class Exerc05 {
	...
	public static void main(String[] args) throws Exception {
		...
		TabelaHash meuHash = new TabelaHash(839);
		...
		while (!id.equals("FIM")) {
			...
			meuHash.pesquisar(pesquisado.getDuration_ms());
			id = MyIO.readLine();
		}
		...
		String path = "\\";
		try {
			File log = new File(path + "722838_hashRehashing.txt");
			log.createNewFile();
		} catch (IOException e) {
		...
		try {
			FileWriter logWriter = new FileWriter(path + "722838_hashRehashing.txt", true);
			logWriter.write(
					"722838\t" + (tempoFinal - tempoInicio) + "\t" + meuHash.comparacao);
			logWriter.close();
		} catch (IOException e) {
