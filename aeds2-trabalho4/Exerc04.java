...
class ABB {
	...
	private Nodo inserir(Nodo raizSubarvore, Musica item) throws Exception {
		...
		if (raizSubarvore == null){
			...
			if (raizSubarvore.getItem().getId().compareTo(item.getId()) > 0) {
...
public class Exerc04 {
	...
	public static void main(String[] args) throws Exception {
		...
		String path = "\\";
		try {
			File log = new File(path + "722838_treeSort.txt");
			log.createNewFile();
		} catch (IOException e) {
		...
		try {
			FileWriter logWriter = new FileWriter(path + "722838_treeSort.txt", true);
