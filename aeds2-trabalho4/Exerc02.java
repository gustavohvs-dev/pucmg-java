...
public class Exerc02 {
	...
	public static void main(String[] args) {
		...
		//Instanciando class pesquisaBinariaRecursiva
		PesquisaBinariaRecursivaClass pesquisaBinariaRecursivaObj = new PesquisaBinariaRecursivaClass();

		Musica[] aux = new Musica[numTotalMusicas];
		int numPosicoes = 0;
		id = MyIO.readLine();
		while (!id.equals("FIM")) {
			pesquisado = pesquisa.buscarId(listaMusicas, id);
			aux[numPosicoes] = pesquisado;
			numPosicoes++;
			id = MyIO.readLine();
		}
		Musica[] vetorMusicas = new Musica[numPosicoes];
		for (int i = 0; i < numPosicoes; i++) {
			vetorMusicas[i] = aux[i];
		}
		sort(vetorMusicas);
		id = MyIO.readLine();
		while (!id.equals("FIM")){
			if(pesquisaBinariaRecursivaObj.pesquisaBinariaRecursiva(vetorMusicas, id)){
		...
		String path = "\\";
		try {
			File log = new File(path + "722838_binaria.txt");
			log.createNewFile();
		} catch (IOException e) {
		...
		try {
			FileWriter logWriter = new FileWriter(path + "722838_binaria.txt", true);
			logWriter.write("722838\t" + (tempoFinal-tempoInicio) + "\t" + pesquisaBinariaRecursivaObj.contadorComparacoes);
			logWriter.close();
		} catch (IOException e) {
    ...
    // QuickSort
    public static void sort(Musica[] musica) {

        quicksort(musica, 0, musica.length - 1);
    }

    private static void quicksort(Musica[] musica, int esq, int dir) {
        int part;
        if (esq < dir) {
            part = particao(musica, esq, dir);
            quicksort(musica, esq, part - 1);
            quicksort(musica, part + 1, dir);
        }
    }

    private static int particao(Musica[] musica, int inicio, int fim) {
        Musica pivot = musica[fim];
        int part = inicio - 1;
        for (int i = inicio; i < fim; i++) {
            if (musica[i].getName().compareTo(pivot.getName()) < 0) {
                part++;
                swap(musica, part, i);
            }
        }
        part++;
        swap(musica, part, fim);
        return (part);
    }

    private static void swap(Musica[] musica, int i, int j) {
        Musica temp = musica[i];
        musica[i] = musica[j];
        musica[j] = temp;
    }
...
class PesquisaBinariaRecursivaClass{

	public int contadorComparacoes = 0;

	public boolean pesquisaBinariaRecursiva(Musica[] musicas, String x) {
		return pesquisaBinariaRecursiva(musicas, x, 0, (musicas.length - 1));
	}

	public boolean pesquisaBinariaRecursiva(Musica[] musicas, String x, int esq, int dir) {
		...
		if (esq > dir) {
		 resp = false;
		} else if (x.compareTo(musicas[meio].getName()) == 0) {
		 resp = true;
		} else if (x.compareTo(musicas[meio].getName()) > 0) {
		 resp = pesquisaBinariaRecursiva(musicas, x, meio + 1, dir);
		} else {
		 resp = pesquisaBinariaRecursiva(musicas, x, esq, meio - 1);
		}
		return resp;
	}
